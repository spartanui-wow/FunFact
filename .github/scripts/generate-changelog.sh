#!/bin/bash
# FunFact Smart Changelog Generator with AI Summaries
#
...
generate_ai_summary_gemini() {
    ...
    if [ "$prompt_type" = "month" ]; then
        local prompt="You are writing release notes for a World of Warcraft addon called FunFact that displays random fun facts. Summarize the following changelog from the last month in 2-3 short sentences. Write for a 6th grade reading level. Focus on the most important changes users care about. Be concise and friendly.

Changes from last month:
$commits_text"
    else
        local prompt="You are writing release notes for a World of Warcraft addon called FunFact that displays random fun facts. Summarize this specific release in 1-2 short sentences. Write for a 6th grade reading level. Focus on what changed in THIS release only. Be concise and friendly.
...
log_info "Generating smart changelog for FunFact..."
...
echo "# FunFact Changelog" > "$OUTPUT_FILE"
echo "" >> "$OUTPUT_FILE"
